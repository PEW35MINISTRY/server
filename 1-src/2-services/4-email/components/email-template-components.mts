...
export const htmlSummaryList = (details:[string, string][], title?:string):string => `${title ? htmlTitle(title) : ''}<div >${details.map(([label, text]) => `<div style="font-family:${EMAIL_FONT_FAMILY.TEXT}; font-size:${EMAIL_FONT_SIZE.TEXT}; color:${EMAIL_COLOR.BLACK}; line-height:${getEmailLineHeight(EMAIL_FONT_SIZE.TEXT)}; text-align:left; margin:5px 0px;"><strong style="color:${EMAIL_COLOR.BLACK}">${label}</strong> ${text}</div>`).join('')}</div>`;

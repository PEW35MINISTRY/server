    ...
    if(html) 
        return [
            htmlDetailList([
                ...
                ['Runtime Duration:', formatDuration(SERVER_START_TIMESTAMP)]
            ], 'Alert Summary'),
            htmlVerticalSpace(5),
...
export const assembleDailyLogReport = async(type:LogType = LogType.ERROR):Promise<EmailReportContent> => {
    ...
    return {subject: `EP ${nowDayOfWeek}'s ${makeDisplayText(type)} Report - ${makeDisplayText(getEnvironment())}`, isHTML:false,
        body: `${nowDayOfWeek} ${makeDisplayText(type)} Report\n`.toUpperCase()
            ...
            + '\n'
            + `Report Range: ${new Date(startTimestamp).toLocaleString('en-US', { timeZone:'America/Chicago' })} - ${new Date(nowTimestamp).toLocaleString('en-US', { timeZone:'America/Chicago' })}\n`
            + '\n'
